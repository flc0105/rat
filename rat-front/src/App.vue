...
>
    ...
    <el-dialog
        v-model="agentOutputsDialogVisible"
        title="Agent Outputs"
        width="1280px"
        top="4vh"
...
>
        <div class="agent-outputs-toolbar-actions" style="margin-bottom: 12px">
            <el-button type="primary" plain class="toolbar-btn" size="small" @click="openAgentBuilderDialog">
                Build Agent
            </el-button>
            <el-button @click="loadAgentOutputs" class="toolbar-btn" size="small" :loading="agentOutputsLoading">
                Refresh
            </el-button>
        </div>
            ...
            <el-table-column label="File" min-width="320">
                <template #default="{ row }">
                    <div class="agent-output-file-cell">
                        <div class="agent-output-file-name" :title="row.file_name">{{ row.file_name || '-' }}</div>
                    </div>
                </template>
            </el-table-column>
       ...
       {{ row.builder || '-' }}
            ...
            <el-table-column label="OS" min-width="100">
                <template #default="{ row }">
                        ...
                        <el-tag size="small" effect="plain">{{ describeAgentTargetOs(row.target_os) }}</el-tag>
                ...
                <template #default="{ row }">
                         ...
                         <el-tag size="small" type="info" effect="plain">{{ row.target_arch || '-' }}</el-tag>
            ...
            <el-table-column label="Socket" min-width="180" show-overflow-tooltip>
                <template #default="{ row }">
                    <span class="agent-output-mono">{{ formatAgentListenerText(row) }}</span>
                </template>
            </el-table-column>

            <el-table-column label="Web" min-width="220" show-overflow-tooltip>
                <template #default="{ row }">
                    <span class="agent-output-mono">{{ formatAgentWebListenerText(row) }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="Actions" width="170" fixed="right">
                <template #default="{ row }">
                    <div class="agent-output-actions">
                        <a class="table-action-link" :href="row.download_url" target="_blank">Download</a>
                        <a class="table-action-link danger"
