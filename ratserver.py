...
class Server:
    ...
    def _register_connection(self, transport: ClientTransport, addr, info: dict) -> ClientSession:
        session = self.web_service.connection_api.create_web_connection(transport, addr, info)
        self.connections.add(session)
        logger.info('Connection has been established: {}'.format(addr))
        self.web_service.connection_api.handle_connection_registered(session)
        return session
    ...
    def _handle_connection_closed(self, session: ClientSession):
        ...
        self.web_service.connection_api.handle_connection_closed(session)
