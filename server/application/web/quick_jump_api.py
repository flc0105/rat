class WebQuickJumpApi:
    ...
    def __init__(self, server, quick_jump_store):
        self.server = server
    ...
    # add hostname quick jump 存储 2026-04-09 15:30
    def list_quick_jumps(self, client_id: str) -> dict:
    ...
    # add hostname quick jump 存储 2026-04-09 15:30
    def save_quick_jump(self, client_id: str, display_name: str, path: str) -> dict:
        hostname = self._get_hostname(client_id)
        item = self.quick_jump_store.save_item(hostname, display_name, path)
        return {
            'hostname': hostname,
            'item': item,
            'items': self.quick_jump_store.list_items(hostname),
    ...
    def delete_quick_jump(self, client_id: str, display_name: str) -> dict:
        hostname = self._get_hostname(client_id)
        item = self.quick_jump_store.delete_item(hostname, display_name)
        return {
            'hostname': hostname,
            'item': item,
            'items': self.quick_jump_store.list_items(hostname),
