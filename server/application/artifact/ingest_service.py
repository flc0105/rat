...
class ArtifactIngestService:
    ...
    def receive_artifact_file(self, command_id, filename, length):
        ...
        """
        file_context = self.session.runtime.pop_file_receive_context(command_id)
        if not isinstance(file_context, FileReceiveContext):
            file_context = FileReceiveContext.from_dict(None)

        on_file_saved = file_context.on_file_saved or self.session.context.on_file_saved
        capture_result = file_context.capture_result

        artifact_service = self.session.context.artifact_service
        if artifact_service is None:
            self.session.send_signal(0, command_id)
            return 0, 'Artifact service is not configured'

        original_name = self._resolve_original_name(filename)
        hostname = self.session.info.get('hostname') or 'unknown_host'
        client_id = self.session.info.get('id') or ''
        addr = self.session.info.get('addr') or ''
        ...
        try:
            status, error = self.session.services.file_receiver.receive_to_io(command_id, length, io)
            if status != 1:
                self._rollback_artifact_files(file_path, meta_path)
                return 0, f'Error receiving file from {self.session.address}: {error}'
            ...
            self._rollback_artifact_files(file_path, meta_path)
            return 0, f'Error receiving file from {self.session.address}: {e}'
    ...
    def _append_file_history(self, command_id: int, artifact_info: dict):
        ...
        将收到的文件挂到对应执行记录上
        """
        self.session.append_file_to_history(command_id, artifact_info)
