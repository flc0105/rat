...
class ServerWebService:
    ...
    def build_connection(self, conn, addr, info: dict):
        return self.connection_service.build_connection(conn, addr, info)

    def on_connection_registered(self, connection):
        self.connection_service.on_connection_registered(connection)

    def on_connection_closed(self, conn):
        self.connection_service.on_connection_closed(conn)

    # ------------------ task facade ------------------ #
    def submit_command(self, client_id: str, command: str):
        return self.task_service.submit_command(client_id, command)

    def submit_upload(self, client_id: str, local_path: str, display_name: str):
        return self.task_service.submit_upload(client_id, local_path, display_name)