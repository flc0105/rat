...
void gb_to_utf8(const char* src, char* dst, int len)
{
	...
	str = (WCHAR*)malloc(i * 2);
	MultiByteToWideChar(CP_ACP, 0, src, -1, str, i);
	i = WideCharToMultiByte(CP_UTF8, 0, str, -1, NULL, 0, NULL, NULL);
	if (len >= i)
	{
		ret = WideCharToMultiByte(CP_UTF8, 0, str, -1, dst, i, NULL, NULL);
		dst[i] = 0;
	}
	if (ret <= 0)
	{
...
void send_result(SOCKET socket, int status, string body) {
	...
	writer.Key("length");
	char src[1024];
	strcpy_s(src, body.c_str());
	char dst[1024];
	gb_to_utf8(src, dst, sizeof(src));
	writer.Int(strlen(dst));
	writer.EndObject();
	string head = s.GetString();
	int head_size = head.size();
	send(socket, (char*)&head_size, sizeof(int), 0);
	send(socket, head.c_str(), head.size(), 0);
	send(socket, dst, strlen(dst), 0);
}
...
string get_os() {
	...
	RegGetValue(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", "ProductName", RRF_RT_ANY, NULL, (PVOID)&value, &buffer_size);
	return value;
}
...
/// <returns></returns>
string get_info() {
	map<string, string> info = { {"os", get_os()}, {"hostname", get_hostname()}, {"integrity", ""}};
	StringBuffer s;
	Writer<StringBuffer> writer(s);
	writer.StartObject();
	for (auto& item : info) {
